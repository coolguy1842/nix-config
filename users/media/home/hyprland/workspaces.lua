...
for _, options in ipairs(WORKSPACES) do
    ...
    hl.bind(string.format("%s + %d", MAIN_MODIFIER, WORKSPACE_ID), hl.dsp.focus({ workspace = tostring(WORKSPACE_ID) }))
    hl.bind(string.format("%s + SHIFT + %d", MAIN_MODIFIER, WORKSPACE_ID), hl.dsp.window.move({ workspace = tostring(WORKSPACE_ID) }))
